...
int main(int argc, char *argv[]){
	...
	int *cellidnum;
	FILE *fin[3]; //0:lai 1:dem 2:coordination 
	...
	if(argc < 2){
		fprintf(stderr,"usage:%s <cellid_asc> <lai_asc> <dem_asc> <coordinate_info> <climdata_dir_with/> <out_base> <year_start>\n",argv[0]);
		return 1;
	}
	...
	for(i=0;i<cells;i++){
		if(cellidnum[i] != -9999.0){
			...
			fprintf(fout,"%f x_coordinate\n",data[i][0]);
			fprintf(fout,"%f y_coordinate\n",data[i][1]);
